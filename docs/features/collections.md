...
While [`group by`](grouping.md) performs SQL-level aggregation (combining rows with `count(*)`), `collect by` creates **visual groupings** — the data is drawn as separate series on the same plot, one for each distinct value of the collection column.
